...
function vectorSearchStage(queryVector, schema, numCandidates, limit, enablePrefilter = false, query = null) {
    const stage = {
        $search: {
            index: '',
            vectorSearch: {
    ...
    if (enablePrefilter && query) {
        stage.$search.vectorSearch.filter = lexicalCompound(query, schema);
    }
